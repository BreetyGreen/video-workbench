...
正式版本发布后，普通用户从 [GitHub Releases](https://github.com/BreetyGreen/video-workbench/releases) 下载与系统对应的同步助手。若没有发布条目，说明还没有公开安装包，不能把 CI 构建成功当作用户已经可下载。管理员在工作台“配置助手 → 服务器交付”生成十分钟一次性配对码；用户首次安装时输入一次，以后登录系统自动监听，不需要 Codex、Python 或仓库。

Mac 构建新增 `VideoWorkbench-Sync-Setup-macOS.zip`，解压后运行 `VideoWorkbench Sync Setup.app`，原生窗口要求 HTTPS 服务器地址和配对码。只配对、不在安装临时目录领取作业；成功后安装登录自启动项。升级失败会恢复旧程序、凭据和启动配置，恢复失败则保留备份并提示。构建支持签名/公证，但未提供 Apple 身份的构建仍为开发测试包，不承诺 Gatekeeper 直接放行。当前 Mac 安装程序包针对构建机器架构，不应称为 Universal 包。

### 普通用户每天怎么用

1. 管理员部署一个长期在线的 HTTPS 服务器、配置需要的云服务。用户不用安装 Codex 或在电脑部署整个后端。
2. 用户电脑安装并打开一次剪映，安装同步助手并输入一次配对码。操作系统要求的文件权限由用户本人允许。
3. 打开工作台侧栏“课程自动剪辑”，上传教学视频或 TXT 讲义，以及有权使用的配套视频；钉钉组织连接器真正配置后接收的课程也会出现在列表。
4. 选择课程、要剪的素材和接收电脑，写一句要求。个人学习与商业用途分开；购买课程不自动获得配套素材的商用权。
5. 可先不启用每日执行，保存计划后点“立即生成”。缺少学习配方时先提取教程规则，有规则才进入剪辑；质检通过后发给目标电脑，不需要强制点审核批准。
6. 验收满意后开启每日执行并选择时间。服务器保持在线，电脑登录、联网、同步助手运行时接收草稿；电脑离线会保留待交付，不表示服务器失败。最后打开剪映检查时间线，发布到平台仍是另一项权限与操作。

同一计划每天最多生成一次，重复点击不会重复扣费生成。修改课程/素材/要求时，新建计划并暂停旧计划。每天复用固定的素材集合，不承诺自动新增素材或每日成片一定不同。服务器停机不补跑过去日期；任务中断会明确显示，不自动重复执行。暂停计划不取消已排队任务。

目前真实钉钉组织网盘、真实付费长课程、公开 HTTPS 服务器和 Mac 剪映客户端完整验收仍需对应资源，不能以演示课或 Windows 本机验证代替。当前部署是一个受控工作区，不是多租户 SaaS。
